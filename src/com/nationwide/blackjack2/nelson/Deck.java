...
public class Deck {
	...
	/**
	 * Constructor that defines the number of decks and whether it should be
	 * shuffled.
	 * 
	 * @param numDecks
	 *            number of individual decks in dealers deck
	 * @param shuffle
	...
	public Deck(int numDecks, boolean shuffle) {
		...
		for (int d = 0; d < numDecks; d++) // for each deck
		{
			for (int s = 0; s < 4; s++) // for each suit
			{
				...
				{
					this.myCards[c] = new Card(Suit.values()[s], n); // add card
																		// to
																		// deck
					c++;
				}
	...
	public void shuffle() {
		...
		for (int i = 0; i < this.numCards; i++) {
			j = rng.nextInt(this.numCards); // get random card j to swap i's
											// value
											// swap cards:
	...
	public Card dealNextCard() {
		...
		this.myCards[this.numCards - 1] = null; // clear out last card to remove
												// it
