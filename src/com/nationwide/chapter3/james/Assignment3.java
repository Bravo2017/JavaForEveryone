...
public class Assignment3 {
	...
	public static void main(String[] args) {
		...
		while (!gameOver) {
			...
			do {
				System.out
						.print("   Your turn (Enter s for scissor, p for paper, t for stone, q to quit): ");
				char inChar = in.next().toLowerCase().charAt(0); // Convert to
																	// lowercase
																	// and
																	// extract
																	// first
			...
			if (!gameOver) {
				// Computer Move
				int aRandomNumber = random.nextInt(3); // random int between 0
														// (inclusive) and 3
														// (exclusive)
				if (aRandomNumber == 0) {
				...
				if (computerMove == playerMove) {
					...
					++numTie;
				} else if (computerMove == HandSign.SCISSOR
						&& playerMove == HandSign.PAPER) {
					...
					++numComputerWon;
				} else if (computerMove == HandSign.STONE
						&& playerMove == HandSign.SCISSOR) {
		...
		System.out.printf("I won %d(%.2f%%). You won %d(%.2f%%).%n",
